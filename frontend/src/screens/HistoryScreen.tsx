        ...
        <div className="space-y-4">
          {filteredCases.map((woundCase) => (
            <div key={woundCase.id} className="relative">
              ...
              {/* Delete controls — overlaid top-right */}
                ...
                {confirmDeleteId === woundCase.id ? (
                  <>
                    <button
                      ...
                      className="text-xs text-white bg-red-500 px-2 py-1 rounded-lg font-semibold shadow"
                    >
                      {deletingCaseId === woundCase.id ? 'Deleting…' : '✓ Confirm'}
                    </button>
                    <button
                  ...
                  <button
                    onClick={(e) => handleDeletePress(e, woundCase.id)}
                    className="w-8 h-8 bg-red-50 rounded-lg flex items-center justify-center text-red-400 hover:bg-red-100 hover:text-red-600 transition-colors shadow-sm"
                    title="Delete case"
                  >
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-gray-500 max-w-xs mb-6">
            {searchQuery
              ...
              : 'Create a case to start tracking a specific wound\'s healing progress.'}
          </p>
          {!searchQuery && (
            <button
              onClick={() => setShowNewCaseModal(true)}
