...
export function ChatAssistantScreen({ onNavigate }: ChatAssistantScreenProps) {
  ...
  const handleSelectQuestion = (question: string) => {
    ...
    // Auto-translate if keys are available
    if (response.messageKey) {
      response.message = t(response.messageKey as any);
    }
    if (response.disclaimerKey) {
      response.disclaimer = t(response.disclaimerKey as any);
    }
  ...
  const getCategoryKey = (category: string) => {
    switch (category) {
      case 'Basic Care': return 'catBasicCare';
      case 'Symptoms & Signs': return 'catSymptomsSigns';
      case 'Infection Concerns': return 'catInfectionConcerns';
      case 'Stitches & Sutures': return 'catStitchesSutures';
      case 'Pain Management': return 'catPainManagement';
      case 'Activity & Lifestyle': return 'catActivityLifestyle';
      case 'Healing & Timeline': return 'catHealingTimeline';
      case 'Nutrition & Factors': return 'catNutritionFactors';
      case 'Scar Care': return 'catScarCare';
      case 'Emergency & Medical': return 'catEmergencyMedical';
  ...
  return (
    <div className="min-h-screen bg-[#F9FBFF] flex flex-col transition-colors">
      {/* Header */}
      <div className="bg-white shadow-sm p-4 flex items-center gap-4 sticky top-0 z-10 transition-colors">
        <button
          onClick={handleBack}
        ...
        <div>
          <h1 className="font-semibold text-gray-800 text-lg">{t('frequentQuestions')}</h1>
          <p className="text-xs text-gray-500">
            {currentQuestion ? t('aiResponse') : selectedCategory ? t(getCategoryKey(selectedCategory) as any) : t('selectCategory')}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto pb-24">
        ...
        {!selectedCategory && !currentResponse && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="bg-gradient-to-br from-[#2F80ED] to-[#6366f1] rounded-2xl p-6 text-white mb-6 shadow-lg shadow-blue-100">
              <h2 className="text-xl font-bold mb-2">{t('howCanWeHelpShort')}</h2>
              <p className="text-blue-100 text-sm">{t('chatIntroDesc')}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {categories.map((category) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-sm leading-tight">{t(getCategoryKey(category) as any)}</h3>
                  <p className="text-[10px] opacity-70 mt-1 uppercase tracking-wider">
                    {t('questionsCount', { count: categorizedQuestions[category].length })}
                  </p>
                </button>
              ))}
            </div>
            ...
            <div className="mt-8 p-4 bg-amber-50 rounded-2xl border border-amber-100 flex gap-3">
              ...
              <div>
                <h4 className="text-amber-900 font-bold text-sm">{t('needImmediateHelp')}</h4>
                <p className="text-amber-800 text-xs mt-1">{t('immediateHelpDesc')}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {selectedCategory && !currentResponse && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            <div className={`p-4 rounded-2xl mb-6 flex items-center gap-4 ${getCategoryColor(selectedCategory)}`}>
              ...
              <h2 className="text-lg font-bold">{t(getCategoryKey(selectedCategory) as any)}</h2>
            </div>
            ...
            <div className="space-y-3">
              {categorizedQuestions[selectedCategory].map((question, index) => (
                <button
                  ...
                  className="w-full bg-white p-5 rounded-2xl shadow-sm border border-gray-100 text-left flex items-center gap-4 group transition-all active:bg-blue-50"
                >
        ...
        {currentResponse && (
          <div className="animate-in fade-in zoom-in-95 duration-300">
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-6 transition-colors">
              <div className="p-6 bg-gray-50 border-b border-gray-100">
                <p className="text-xs text-gray-400 uppercase font-bold tracking-widest mb-2">{t('questionLabel')}</p>
              ...
              <div className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">{t('aiVerifiedAnswer')}</span>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-400 uppercase tracking-widest font-bold">
                  <span>{t('aiConfidenceLevel', { confidence: currentResponse.confidence })}</span>
                  <span>{t('referenceClinical')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100 mb-6">
              <p className="text-[10px] text-amber-800 leading-relaxed font-medium">
                {t('disclaimerLabel')}: {currentResponse.disclaimer}
              </p>
            </div>
      ...
      {!currentResponse && (
        <div className="p-4 bg-white border-t border-gray-100 fixed bottom-0 left-0 right-0 z-20 transition-colors">
