...
export function HomeScreen({ onNavigate }: HomeScreenProps) {
  ...
  const handleProceed = async () => {
    ...
    try {
      if (mode === 'create') {
        ...
        const newCase = await createNewCase(newCaseName, t('createdFromHome'));
        if (newCase) {
  ...
  return (
    <div className="min-h-screen bg-[#F9FBFF] p-4 pb-20 relative transition-colors">
      {/* Header */}
      <div className="pt-8 pb-6 flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-gray-800">{t('helloStayHealthy')}</h1>
            <p className="text-gray-500 mt-1">{t('howCanWeHelp')}</p>
          </div>
        </div>
        <button
      ...
      >
        ...
        <span className="text-lg font-medium">{t('uploadWound')}</span>
      </button>
      ...
      >
        ...
        <span className="text-lg font-medium">{t('woundHistory')}</span>
      </button>
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-gray-800 text-lg">{t('frequentQuestions')}</h3>
              <p className="text-gray-500 text-sm mt-1">{t('frequentQuestionsDesc')}</p>
            </div>
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-gray-800 text-lg">{t('dailyCareTips')}</h3>
              <p className="text-gray-500 text-sm mt-1">{t('dailyCareTipsDesc')}</p>
            </div>
            <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
      ...
      <div className="mt-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-3">{t('learnMore')}</h2>
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <h4 className="font-medium text-gray-800">{t('woundTypes')}</h4>
            <p className="text-xs text-gray-500 mt-1">{t('woundTypesDesc')}</p>
          </button>
          ...
          >
            ...
            <h4 className="font-medium text-gray-800">{t('visualReference')}</h4>
            <p className="text-xs text-gray-500 mt-1">{t('visualReferenceDesc')}</p>
          </button>
          ...
          >
            ...
            <h4 className="font-medium text-gray-800">{t('healingLevels')}</h4>
            <p className="text-xs text-gray-500 mt-1">{t('healingLevelsDesc')}</p>
          </button>
          ...
          >
            ...
            <h4 className="font-medium text-gray-800">{t('faqs')}</h4>
            <p className="text-xs text-gray-500 mt-1">{t('faqsDesc')}</p>
          </button>
        </div>
      </div>
      ...
      {showCaseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl transition-colors">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">{t('startNewAnalysis')}</h3>
            ...
            {mode === 'create' ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('caseName')}</label>
                <input
                  type="text"
                  value={newCaseName}
                  onChange={(e) => setNewCaseName(e.target.value)}
                  placeholder={t('caseNamePlaceholder')}
                  className="w-full px-4 py-2 bg-white border border-gray-300 rounded-xl text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
            ) : (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('selectCase')}</label>
                <select
