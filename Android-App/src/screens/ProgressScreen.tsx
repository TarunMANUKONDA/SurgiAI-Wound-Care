...
export function ProgressScreen({ onNavigate, patientAnswers, analysis }: ProgressScreenProps) {
    ...
    const loadProgressData = () => {
      ...
      const data: ProgressDataPoint[] = history.map((wound, index) => {
        ...
        return {
          ...
          date: woundDate.toLocaleDateString(settings.language === 'en' ? 'en-US' : settings.language, { month: 'short', day: 'numeric' })
        };
      });
  ...
  const progressLabels = ['riskNormal', 'riskWarning', 'riskInfected', 'riskCritical'];
  ...
  return (
    <div className="min-h-screen bg-[#F9FBFF] p-4 pb-20">
      ...
      <div className="flex items-center gap-4 pt-4 pb-6">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-800">{t('healingProgress')}</h1>
          <p className="text-sm text-gray-500">{t('dayPostSurgery', { count: days })}</p>
        </div>
      </div>

      {/* Analysis Summary Card */}
      {analysis && (
        <div className={`rounded-2xl p-4 shadow-md mb-4 ${analysis.riskLevel === 'normal' ? 'bg-green-50 border border-green-200' :
          ...
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-800">{t('aiAnalysisSummary')}</h3>
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${analysis.riskLevel === 'normal' ? 'bg-green-200 text-green-800' :
                analysis.riskLevel === 'warning' ? 'bg-yellow-200 text-yellow-800' :
                  analysis.riskLevel === 'infected' ? 'bg-red-200 text-red-800' :
                    'bg-red-300 text-red-900'
              }`}>
              {t(('risk' + analysis.riskLevel.charAt(0).toUpperCase() + analysis.riskLevel.slice(1)) as any)}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/60 rounded-lg p-3">
              <p className="text-xs text-gray-500">{t('overallHealth')}</p>
              <p className="text-lg font-bold text-gray-800">{analysis.overallHealth}%</p>
            </div>
            <div className="bg-white/60 rounded-lg p-3">
              <p className="text-xs text-gray-500">{t('edgeQuality')}</p>
              <p className="text-lg font-bold text-gray-800">{analysis.edgeQuality}%</p>
            </div>
            <div className="bg-white/60 rounded-lg p-3">
              <p className="text-xs text-gray-500">{t('rednessLevel')}</p>
              <p className={`text-lg font-bold ${analysis.rednessLevel > 60 ? 'text-red-600' :
                  analysis.rednessLevel > 40 ? 'text-yellow-600' : 'text-green-600'
                }`}>{analysis.rednessLevel}%</p>
            </div>
            <div className="bg-white/60 rounded-lg p-3">
              <p className="text-xs text-gray-500">{t('swellingLevel')}</p>
              <p className={`text-lg font-bold ${analysis.swellingLevel > 50 ? 'text-red-600' :
      ...
      <div className="bg-white rounded-2xl p-4 shadow-md mb-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800">{t('healingProgressChart')}</h3>
          <button
            onClick={() => setShowDetails(!showDetails)}
            className="text-sm text-[#2F80ED] font-medium"
          >
            {showDetails ? t('simpleView') : t('detailedView')}
          </button>
        </div>
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {[
            { key: 'all', label: t('allMetrics') },
            { key: 'health', label: t('health'), color: '#27AE60' },
            { key: 'redness', label: t('redness'), color: '#EB5757' },
            { key: 'healing', label: t('healing'), color: '#2F80ED' }
          ].map((tab) => (
            <button
              key={tab.key}
              onClick={() => setSelectedMetric(tab.key as typeof selectedMetric)}
              className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all ${selectedMetric === tab.key
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="bg-green-50 rounded-xl p-3">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600">{t('health')}</span>
              <div className={`flex items-center gap-1 ${healthTrend.direction === 'up' ? 'text-green-600' :
          ...
          <div className="bg-red-50 rounded-xl p-3">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600">{t('redness')}</span>
              <div className={`flex items-center gap-1 ${rednessTrend.direction === 'down' ? 'text-green-600' :
          ...
          <div className="bg-blue-50 rounded-xl p-3">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600">{t('healing')}</span>
              <div className={`flex items-center gap-1 ${healingTrend.direction === 'up' ? 'text-green-600' :
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-xl">
          <p className="text-xs text-gray-500 mb-2">{t('understandingChart')}</p>
          <div className="flex flex-wrap gap-3 text-xs">
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-full bg-[#27AE60]"></div>
              <span className="text-gray-600">{t('healthBetter')}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-full bg-[#EB5757]"></div>
              <span className="text-gray-600">{t('rednessBetter')}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-full bg-[#2F80ED]"></div>
              <span className="text-gray-600">{t('healingBetter')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-md mb-4">
        ...
        <div className="space-y-3">
          {healingStages.map((stage, index) => (
            <div
              key={stage.stage}
              className={`p-4 rounded-xl border-2 transition-all ${index === currentStageIndex
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl ${index === currentStageIndex
                    ? 'bg-[#2F80ED] text-white'
                    : index < currentStageIndex
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h4 className="font-semibold text-gray-800">{t(stage.stage as any)}</h4>
                    {index === currentStageIndex && (
                  ...
                  <p className="text-sm text-gray-600 mt-1">{t((stage.stage + 'Desc') as any)}</p>
                  {index === currentStageIndex && (
                    <div className="mt-2 p-2 bg-white rounded-lg">
                      <p className="text-xs text-[#2F80ED] font-medium">
                        {t('careSteps')}: {t((stage.stage + 'Care') as any)}
                      </p>
                    </div>
                  )}
                </div>
      ...
      {/* Healing Care Matrix */}
      <div className="bg-white rounded-2xl p-4 shadow-md mb-4">
        <h3 className="font-semibold text-gray-800 mb-3">{t('healingCareMatrix')}</h3>
        <div className="overflow-hidden rounded-xl border border-gray-200">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left p-3 font-medium text-gray-700">{t('healingTimeline')}</th>
                <th className="text-left p-3 font-medium text-gray-700">{t('careSteps')}</th>
              </tr>
            </thead>
            <tbody>
              {healingStages.map((stage, index) => (
                <tr
                  key={stage.stage}
                  className={`border-t border-gray-200 ${index === currentStageIndex ? 'bg-blue-50' : ''
                    }`}
                >
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={index === currentStageIndex ? 'font-medium text-[#2F80ED]' : ''}>
                        {t(stage.stage as any)}
                      </span>
                    </div>
                  </td>
                  <td className="p-3 text-gray-600">{t((stage.stage + 'Care') as any)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {analysis && analysis.detectedFeatures.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-md mb-4">
          <h3 className="font-semibold text-gray-800 mb-3">{t('aiDetectedFeatures')}</h3>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[#2F80ED] font-medium">
            {t('nextSuggestedScan', {
              time:
                analysis?.riskLevel === 'critical' ? t('immediatelyAfterMedical') :
                  analysis?.riskLevel === 'infected' ? t('hoursShort', { h: 12 }) :
                    analysis?.riskLevel === 'warning' ? t('hoursShort', { h: '12-24' }) :
                      t('hoursShort', { h: 24 })
            })}
          </span>
        </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-4">
        <div className="flex items-center justify-between">
          <span className="text-gray-600">{t('aiConfidence')}:</span>
